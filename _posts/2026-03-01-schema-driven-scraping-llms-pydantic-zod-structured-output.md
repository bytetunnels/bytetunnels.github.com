...
**Function calling with schema** is the other approach. You define a function with a JSON Schema for its parameters, and the model returns arguments matching that schema:
...
tools = [
    {
        ...
        "function": {
            ...
            "parameters": ProductPage.model_json_schema(),
            "strict": True,
        },
    }
]
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=[
        {"role": "system", "content": "Extract product data from the HTML."},
        {"role": "user", "content": f"HTML:\n\n{html[:8000]}"},
    ],
    tools=tools,
...
page = ProductPage.model_validate(args)
```

The function calling approach is useful when you want to integrate extraction into an [agent workflow](/posts/browser-agent-frameworks-compared-browser-use-vs-stagehand-vs-skyvern/) where the model decides when and how to extract data. This pairs naturally with [Playwright for browser automation in AI agents](/posts/playwright-for-browser-automation-in-ai-agents/) and [MCP-based tool integration](/posts/playwright-mcp-and-cli-making-browser-automation-ai-agent-friendly/).
...
The equivalent in TypeScript with Zod:
...
const ProductSchema = z.object({
  ...
  originalPrice: z
    ...
    .positive()
    .nullable()
    .describe("Price before discount"),
  rating: z.number().min(0).max(5).nullable().describe("Rating out of 5"),
  reviewCount: z
    .number()
    .int()
    .nonnegative()
...
function pruneHTML(html: string): string {
  ...
  const noise =
    /cookie|consent|popup|modal|sidebar|advert|newsletter/i;
  $("[class]").each((_, el) => {
    if (noise.test($(el).attr("class") || "")) $(el).remove();
  });
  $("[id]").each((_, el) => {
    if (noise.test($(el).attr("id") || "")) $(el).remove();
  });
...
async function extractProducts(url: string): Promise<ExtractionResult> {
  const response = await fetch(url);
  const html = await response.text();
  const pruned = pruneHTML(html);

  console.log(`Pruned: ${pruned.length} chars (from ${html.length})`);
...
// -- Run --
const data = await extractProducts("https://example.com/products");
for (const p of data.products) {
  let discount = "";
  if (p.originalPrice && p.originalPrice > p.price) {
    const pct = ((1 - p.price / p.originalPrice) * 100).toFixed(0);
    discount = ` (${pct}% off)`;
  }
  const stock = p.inStock ? "In stock" : "Out of stock";
  console.log(`  ${p.name}: $${p.price.toFixed(2)}${discount} - ${stock}`);
}
```

Both examples follow the same pattern: define a schema, prune the HTML, call the LLM with structured output constraints, and validate the result. The schema does the heavy lifting -- it tells the LLM what to look for, constrains the output format, and validates the result in a single declaration.
