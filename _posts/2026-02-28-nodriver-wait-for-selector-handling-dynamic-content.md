...
You can extend this to wait for specific text patterns:

```python
import re


async def wait_for_text_match(page, selector, pattern, timeout=10, poll_interval=0.5):
    """Wait until element text matches a regex pattern."""
    compiled = re.compile(pattern)
    start = time.time()
    while time.time() - start < timeout:
        element = await page.select(selector)
        if element and element.text and compiled.search(element.text):
            return element
        await page.sleep(poll_interval)
    raise TimeoutError(
        f"Selector '{selector}' text did not match '{pattern}' within {timeout}s"
    )


# Usage: wait for a price that looks like "$XX.XX"
price = await wait_for_text_match(page, "span.price", r"\$\d+\.\d{2}", timeout=10)
```
...
For critical scraping jobs, wrap the entire page interaction in a retry loop:

```python
async def scrape_with_retry(browser, url, max_retries=3):
    """Retry the entire scraping operation on failure."""
    for attempt in range(max_retries):
        try:
            page = await browser.get(url)
            await wait_for_selector(page, "div.content", timeout=10)

            # Extract data
            items = await page.query_selector_all("div.content .item")
            results = []
            for item in items:
                text = item.text
                if text:
            ...
            print(f"Attempt {attempt + 1}/{max_retries} timed out for {url}")
...
## Complete Example: Scraping a Dynamically-Loaded Page

Here is a full script that ties everything together. It navigates to a page, waits for dynamic content, handles errors, and extracts structured data:
...
async def wait_for_selector(page, selector, timeout=10, poll_interval=0.5):
...
async def wait_for_text(page, selector, timeout=10, poll_interval=0.5):
    """Wait until an element has non-empty text content."""
    start = time.time()
    while time.time() - start < timeout:
        element = await page.select(selector)
        if element and element.text and element.text.strip():
            return element
        await page.sleep(poll_interval)
    raise TimeoutError(f"Selector '{selector}' had no text within {timeout}s")


async def scrape_product_listing(browser, url):
    """Scrape product data from a dynamically-loaded listing page."""
    page = await browser.get(url)

    # Step 1: Wait for the loading indicator to disappear
    try:
        await wait_for_selector_gone(page, "div.loading-overlay", timeout=15)
    except TimeoutError:
        print("Loading overlay did not disappear, continuing anyway")

    # Step 2: Wait for product cards to appear
    try:
        await wait_for_selector(page, "div.product-card", timeout=15)
    except TimeoutError:
        print(f"No products found on {url}")
        await page.save_screenshot("no_products.png")
        return []

    # Step 3: Wait for prices to load (they come from a separate API call)
    try:
        await wait_for_text(page, "div.product-card span.price", timeout=10)
    except TimeoutError:
        print("Prices did not load, extracting what we have")

    # Step 4: Extract all product data using JavaScript for efficiency
    products = await page.evaluate(
        """
        (() => {
            const cards = document.querySelectorAll('div.product-card');
            return Array.from(cards).map(card => ({
                name: card.querySelector('h3.product-name')?.textContent?.trim() || '',
                price: card.querySelector('span.price')?.textContent?.trim() || '',
                rating: card.querySelector('span.rating')?.textContent?.trim() || '',
                url: card.querySelector('a.product-link')?.href || '',
                in_stock: !card.querySelector('.out-of-stock'),
            }));
        })()
        """
    )
...
async def main():
    ...
    for url in urls:
        ...
        try:
            products = await scrape_product_listing(browser, url)
...
The script follows a clear sequence for each page: wait for loading to finish, wait for the main content, wait for supplementary data, then extract. Each wait step has its own timeout and error handling, so a failure at one stage does not crash the entire run.
